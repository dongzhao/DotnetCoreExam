...
namespace Infrastructure.UnitTest
{
    public class OrderEventTest : DomainEventBaseTest
    {
        ...
        public async Task test_add_sync_with_notification(string[] shippingAddress, string[] residentalAddress, string[] cardDetails, string[] recipentDetails, string[] orderDetails)
        {
            ...
            actural1.AddToEvents();

            Assert.IsTrue(actural1.DomainEvents.Count == 1);

            var expected2 = new Order()
            {
                ShippingAddress = address1,
                ResidentAddress = address2,
                CurrentDateTime = DateTime.Parse(orderDetails[0]),
            ...
            var actural2 = await _unitOfWork.OrderRepository.AddAsync(expected2);
            actural2.AddToEvents();

            //Assert.IsTrue(actural1.DomainEvents.Count == 2);
            //Assert.IsTrue(actural2.DomainEvents.Count == 2);

            await _unitOfWork.CommitAndNotifyAsync();

            Assert.IsTrue(actural1.Id > 0);
            Assert.IsTrue(actural2.Id > 0);

            Assert.IsTrue(actural1.DomainEvents.Count == 0);
            Assert.IsTrue(actural2.DomainEvents.Count == 0);
        }
